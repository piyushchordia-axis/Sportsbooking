...
3. **DONE (viewer)** — built the audit-viewer surface: an owner Activity log page + GET /audit-logs (filterable by action/entity, paginated, owner-scoped via RLS, actor-name enriched). The interceptor now also records the sanitized **changed fields** per mutation (metadata.changes, sensitive keys redacted), surfaced in the Activity log. _Still future:_ full before→after diffs (needs per-handler instrumentation).
4. ~~P1 Integrate a real notification provider (WhatsApp/SMS/email)…~~ **DESCOPED** — product decision: notifications are **in-app only** (the bell feed). No external delivery channels.
...
7. ~~P1 Enforce pack expiry/flatRate fully.~~ **DONE** — FORFEIT packs time-expire sessions (expiresAt = purchase + validityDays; ROLLOVER/NONE never expire), and FLAT packs apply `flatRate × sessions` capped at the slot subtotal (null flatRate covers all) — both covered by the memberships spec.
