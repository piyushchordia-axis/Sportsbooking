...
3. **DONE (viewer)** — built the audit-viewer surface: an owner Activity log page + GET /audit-logs (filterable by action/entity, paginated, owner-scoped via RLS, actor-name enriched). _Still future:_ semantic per-handler before/after diffs for high-value actions (the interceptor records route-derived create/update/delete rows, not field-level diffs).
