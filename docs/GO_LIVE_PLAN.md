...
| 2.4 ✅ | **DONE (dues dashboard; forfeited-balance still TODO)** — Added `GET /bookings/dues-summary` (owner/staff-scoped: count + total `amountDueAtVenue` for AWAITING_VENUE_SETTLEMENT bookings) and a "₹X to collect at venue / N bookings awaiting settlement" card on the owner BookingsPage that filters the table to those bookings. Verified: endpoint returns `{count:2, totalDue:1400}` matching a raw SQL cross-check; both apps typecheck. (Browser render not previewed — port 3001 held by an unrelated local project.) The forfeited-deposit-on-cancel surfacing remains a small follow-up. | M |
